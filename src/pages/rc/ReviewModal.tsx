...
export function ScoreSelect({
  ...
  return (
    <select
      ...
      aria-label="Score"
      className={`rounded-lg border border-zinc-300 bg-white px-2.5 py-1.5 text-sm font-medium text-zinc-800 outline-none focus:ring-2 focus:ring-indigo-600 ${className}`}
    >
...
export function ReviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <header className="flex items-center justify-between gap-3 border-b border-zinc-200 px-4 sm:px-6 py-3 safe-top">
        <div>
          <p className="text-base font-semibold text-zinc-900">Application #{c.number ?? '-'}</p>
        ...
        <div className="max-w-3xl mx-auto px-6 py-8">
            ...
            <div className="flex flex-col gap-5">
              ...
              {buildSummaryItems({
                ...
                <div key={i}>
                  ...
                  <p className="text-lg text-zinc-900 whitespace-pre-wrap">{item.value}</p>
                </div>
              ))}
          ...
          {screen.kind === 'answer' && (
            <div className="flex flex-col gap-4">
              ...
              <p className="text-xl font-semibold text-zinc-900 whitespace-pre-wrap">{screen.item.question}</p>
              {screen.item.answer.trim() ? (
                <p className="text-lg leading-relaxed text-zinc-800 whitespace-pre-wrap">{screen.item.answer}</p>
              ) : (
                <p className="text-lg text-zinc-400">No answer provided.</p>
              )}
            </div>
          )}
      ...
      <footer className="flex items-center justify-between border-t border-zinc-200 px-4 sm:px-6 py-3 safe-bottom">
        <button
